...
#define LED 26 //VOWELS
// GPIO pin for red LED
#define LED2 5 //CONSONANTS
// GPIO pin for button
#define BUTTON 19
// =======================================================
...
static const int colors = COLS;  //assigns colours teh number 3 as there are 3 colours being used (R,G,B)
static const int seqlen = SEQL; //assigns a sequence length of 3
static int exactMatches=0;
static int approxMatches=0;
...
extern void waitForButton (uint32_t *gpio, int button);
...
void blinkN(uint32_t *gpio, int led, int c) {  
  /* ***  COMPLETE the code here  ***  */ //led refers to the pin number 
  //during blinking switch the led between high or low 
  for (int i=0; i<c; i++){
    //set the led
...
void initSeq() {
  ...
  for (int i= 0; i<SEQL; i++){
    theSeq[i] = (rand() % COLS) + 1;  //generates a random sequence of 3 digit numbers
  }
}
...
//   //create copies of the sequences 
//   int cpy1[seqlen];
//   int cpy2[seqlen];

//   for (int i=0; i<=3; i++){
//     cpy1[i]=seq1[i] //copies the contents of the secret sequence into cpy1
//     cpy2[i]=seq2[i] //copies the contents of the guess into cpy2
//   }

//   //checks for the exact matches 
//   for (int i=0; i<3; i++){
//     if cpy1[i] == cpy2[i]{ //there is an exact match at this index
//         cpy1[i] = NAN1;  //allocates a unique number to be used at this index to indicate that there was an exact match
//         cpy2[i] =NAN1; 
//         exact++;
//     } 
//   }

//   //checking for approximate matches
//   for (int i =0; i<3; i++){
//     if cpy2[i]!=NAN1{
//         for (int j =0; j<3; j++){
//             if cpy2[i] = cpy1[i]{
...
//   int *results = (int*)malloc(sizeof(int)*2) //to hold the exact matches and approximate matches 
//   results[0] = exact;
//   results[1]= approximate;
//   return results;


// }

extern int matches(int* secret, int * guess );  // bahseer added this to avoid warning with matches function (idk what i am doing)

/* counts how many entries in seq2 match entries in seq1 */
/* returns exact and approximate matches, either both encoded in one value, */
/* or as a pointer to a pair of values */
int /* or int* */ countMatches(int *seq1, int *seq2) {
  // Create copies of seq1 and seq2
  int cpy1[seqlen];
  int cpy2[seqlen];
  ...
  // Call the assembly function with the copies
  int result = matches(cpy1, cpy2);

  // Return the result
  return result;
}
/* show the results from calling countMatches on seq1 and seq1 */ //adjust teh code 
...
void readSeq(int *seq, int val) {  //val is a digit and seq is the array 
  /* ***  COMPLETE the code here  ***  */
  if (seq==NULL){
    fprintf(stderr,"Failed to allocate memory");
  }
  for (int i=2; i>=0; i--){
    seq[i]= val % 10; //stores the value in the ones position in seq[i]
    val= val/10;
  }
...
void timer_handler (int signum) {
  /* ***  COMPLETE the code here  ***  */
  static int count = 0;
  printf("Timer expired %d times. Current time: %llu microseconds\n", ++count, timeInMicroseconds());
}
...
void initITimer(uint64_t timeout){
  ...
  // Configure the timer to expire after timeout microseconds
  timer.it_value.tv_sec = timeout / 1000000;
  timer.it_value.tv_usec = timeout % 1000000;
  // and every timeout microseconds after that
...
int main (int argc, char *argv[]){ // this is just a suggestion of some variable that you may want to use
  ...
  if (opt_s) { // if -s option is given, use the sequence as secret sequence
    if (theSeq==NULL) 
      theSeq = (int*)malloc(seqlen*sizeof(int)); //allocates memory to the sequence 
    readSeq(theSeq, opt_s); //takes the digits in the secret code and stores it in the Seq
    if (verbose) {
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  void greeting(uint32_t *gpio, const char *surname) {
    if (gpio == NULL) {
        fprintf(stderr, "Error: GPIO not initialized\n");
        exit(EXIT_FAILURE);
    }
    lcdClear(lcd); // Clear the LCD before displaying the name
    lcdPuts(lcd, "Hello, ");
    lcdPuts(lcd, surname); // Display the surname on the LCD
  ...
  /* ***  COMPLETE the code here  ***  */ //PRINT NAME
  // Personalized greeting
  const char *surname = "Menezes"; // Replace with the family name
  greeting(gpio, surname);
  /* initialise the secret sequence */
  if (!opt_s) 
    initSeq(); //if the secret sequence doesnt exist then create a sequence 
  if (debug) //if debug mode is chosen
    showSeq(theSeq); //show the sequence in the terminal 

  if (theSeq == NULL) {
      fprintf(stderr, "Error: theSeq is NULL\n");
      exit(EXIT_FAILURE);
  }  
  ...
  // -----------------------------------------------------------------------------
  // +++++ main loop
//   while (!found && attempts < MAX_ATTEMPTS) { //while the sequence is not found 
//     attempts++;
//     for (i = 0; i < seqlen; i++) {
//       int count = 0;
...
//   // int code = (matches_m[1] << 8) | matches_m[0];
